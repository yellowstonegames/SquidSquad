...
public class MarkovTextLimited {
     ...
     * call any other methods. You can serialize this MarkovTextLimited after calling to avoid needing to call this again on later
     * runs, or even include serialized MarkovTextLimited objects with a game to only need to call this during pre-processing.
    ...
    {
        NumberedSet<String> body = new NumberedSet<>(corpus.length() / 5 + 5);
        body.add(INITIAL);
        body.add(FULL_STOP);
        body.add(EXCLAMATION);
        body.add(QUESTION);
        body.add(ELLIPSIS);
        ArrayList<IntList> working = new ArrayList<>(corpus.length() / 5 + 5);
        ...
        IntList w = working.get(previous), v;
    ...
    /**
     * After calling {@link #analyze(CharSequence)}, you can optionally call this to alter any words in this MarkovTextLimited that
     ...
     * especially fast if the corpus was large). This method modifies this MarkovTextLimited in-place.
     * @param translator a Translator that will be used to translate proper nouns in this MarkovTextLimited's word array
     */
    ...
    public <S extends CharSequence & Appendable> S appendTo(S sb){
        ...
        Base.BASE10.appendJoined2D(sb, "$", "|", processed);
        return sb;
    }
    ...
    /**
     * Copies the String array {@link #words} and the 2D jagged int array {@link #processed} into a new MarkovTextLimited.
     * None of the arrays will be equivalent references, but the Strings (being immutable) will be the same objects in
     * both MarkovTextLimited instances. This is primarily useful with {@link #changeNames(Translator)}, which can
     * produce several variants on names given several initial copies produced with this method.
     * @return a copy of this MarkovTextLimited
     */
