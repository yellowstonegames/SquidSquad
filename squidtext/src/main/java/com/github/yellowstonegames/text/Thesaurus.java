...
public class Thesaurus {
     ...
     * Changes the sequences for all groups of synonyms this can produce, effectively turning this Thesaurus into a
     * different version that knows all the same synonyms and categories but will produce different results.
     * This version of refresh() is meant for cases where the generator this uses may have four or more states, or when
     * you don't know how many long items will be in {@code states}.
     * @param states a non-null long array, usually with length equal to {@link EnhancedRandom#getStateCount()}
     */
    public void refresh(long... states)
    {
        this.rng.setState(states);
        final int sz = mappings.size();
        for (int i = 0; i < sz; i++) {
            mappings.getAt(i).setRNG(rng, true);
        }
     ...
     * <br>
     * "lovecraft`pre`", "english`pre`", "greek_romanized`pre`", "greek_authentic`pre`", "french`pre`",
     * "russian_romanized`pre`", "russian_authentic`pre`", "japanese_romanized`pre`", "swahili`pre`", "somali`pre`",
     * "hindi_romanized`pre`", "arabic_romanized`pre`", "inuktitut`pre`", "norse`pre`", "nahuatl`pre`",
     * "mongolian`pre`", "fantasy`pre`", "fancy_fantasy`pre`", "goblin`pre`", "elf`pre`", "demonic`pre`",
     * "infernal`pre`", "simplish`pre`", "alien_a`pre`", "korean_romanized`pre`", "alien_e`pre`", "alien_i`pre`",
     * "alien_o`pre`", "alien_u`pre`", "dragon`pre`", "kobold`pre`", "insect`pre`", "maori`pre`", "spanish`pre`",
     * "deep_speech`pre`", "norse_simplified`pre`", "hletkip`pre`", "ancient_egyptian`pre`", "crow`pre`", "imp`pre`",
     * "malay`pre`", "celestial`pre`", "chinese_romanized`pre`", "cherokee_romanized`pre`", "vietnamese`pre`",
     * "latin`pre`"
     * <br>
     * These correspond to similarly-named fields in {@link Language}, just with {@code `pre`}; for instance
    ...
    public Thesaurus addFakeWords()
    {
        ...
        {
            ObjectList<String> words = new ObjectList<>(100);
            for (int i = 0; i < 100; i++) {
