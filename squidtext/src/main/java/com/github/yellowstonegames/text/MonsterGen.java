...
public class MonsterGen implements ISerializersNeeded {
    ...
    public String[] components =
            new String[]{
                    "head", "tail", "legs", "claws", "fangs", "eyes", "hooves", "beak",
                    "wings", "pseudopods", "snout", "carapace", "sting", "pincers", "fins", "shell"},
            adjectives = new String[]{
                    "hairy", "scaly", "feathered", "chitinous", "pulpy", "writhing", "horrid", "fuzzy", "reptilian",
                    "avian", "insectoid", "tentacled", "thorny", "angular", "curvaceous", "lean", "metallic", "stony",
                    "glassy", "gaunt", "obese", "ill-proportioned", "sickly", "asymmetrical", "muscular"},
            powerAdjectives = new String[]{
                    "fire-breathing", "electrified", "frigid", "toxic", "noxious", "nimble", "brutish", "bloodthirsty",
                    "furious", "reflective", "regenerating", "earth-shaking", "thunderous", "screeching", "all-seeing",
                    "semi-corporeal", "vampiric", "skulking", "terrifying", "undead", "mechanical", "angelic",
                    "plant-like", "fungal", "contagious", "graceful", "malevolent", "gigantic", "wailing"},
            powerPhrases = new String[]{
                    "can evoke foul magic", "can petrify with its gaze", "hurts your eyes to look at", "can spit venom",
                    "can cast arcane spells", "can call on divine power", "embodies the wilderness",
                    "hates all other species", "constantly drools acid",
                    "whispers maddening secrets in forgotten tongues", "shudders between impossible dimensions",
    ...
    public static class Chimera implements ISerializersNeeded
    {
        ...
        public Chimera(String name, Chimera other)
        {
            ...
            powerAdjectives = new ObjectOrderedSet<>(other.powerAdjectives);
            powerPhrases = new ObjectOrderedSet<>(other.powerPhrases);
        }
        ...
        public Chimera(String name, String unknown, String... terms)
        {
            ...
            if(unknown != null)
                ...
                mainForm = name;
            parts = new ObjectObjectOrderedMap<>();
            unsaidAdjectives = new ObjectOrderedSet<>();
        ...
        public String present(boolean capitalize)
        {
            ...
            if(capitalize)
                ...
                sb.append('a');
            int i = 0;
            ObjectOrderedSet<String> allAdjectives = new ObjectOrderedSet<>(wholeAdjectives);
            if(unknown != null)
        ...
        /**
         * Fuse two Chimera objects by some fraction of influence, using the given EnhancedRandom and possibly renaming the
         * creature. Does not modify the existing Chimera objects.
         * @param rng the EnhancedRandom to determine random factors
         ...
         */
        public Chimera mix(EnhancedRandom rng, String newName, Chimera other, double otherInfluence)
        {
            Chimera next = new Chimera(newName, this);
            ObjectList<String> otherParts = new ObjectList<>(other.parts.get(other.name));
            rng.shuffle(otherParts);
            otherParts.truncate((int)Math.round(otherParts.size() * otherInfluence * 0.5));
            next.parts.put(other.name, otherParts);
     ...
     */
    public Chimera randomizeAppearance(EnhancedRandom rng, Chimera creature, String newName, int adjectiveCount)
    {
        Chimera next = new Chimera(newName, creature);
        Collections.addAll(next.wholeAdjectives, portion(rng, Arrays.copyOf(adjectives, adjectives.length), adjectiveCount));
    ...
    {
        return randomizeAppearance(random, creature, newName, adjectiveCount);
    }
     ...
     */
    public Chimera randomizePowers(EnhancedRandom rng, Chimera creature, String newName, int powerCount)
    {
        Chimera next = new Chimera(newName, creature);
        int adjs = rng.nextInt(powerCount + 1), phrs = powerCount - adjs;
        Collections.addAll(next.powerAdjectives, portion(rng, Arrays.copyOf(powerAdjectives, powerAdjectives.length), adjs));
        Collections.addAll(next.powerPhrases, portion(rng, Arrays.copyOf(powerPhrases, powerPhrases.length), phrs));
        return next;
    }
    ...
    {
        return randomizePowers(random, creature, newName, powerCount);
    }
    ...
    public Chimera randomize(EnhancedRandom rng, String newName, int detail)
    {
        ...
        if(detail > 0) {
            ...
            Collections.addAll(next.unsaidAdjectives, portion(rng, Arrays.copyOf(adjectives, adjectives.length), bodyCount));
            Collections.addAll(next.powerAdjectives, portion(rng, Arrays.copyOf(powerAdjectives, powerAdjectives.length), adjs));
            Collections.addAll(next.powerPhrases, portion(rng, Arrays.copyOf(powerPhrases, powerPhrases.length), phrs));
        }
        return next;
    ...
    {
        return randomize(random, newName, detail);
    }
    ...
    {
        return randomize(random, randomName(random), 5);
    }

    /**
     * Gets a random name as a String using FakeLanguageGen.
     * @param rng the EnhancedRandom to use for random factors
     * @return a String meant to be used as a creature name
     */
    public String randomName(EnhancedRandom rng)
    {
        return Language.FANTASY_NAME.word(rng, false, rng.nextInt(2, 4));
    }
    ...
    private static String[] portion(EnhancedRandom rng, String[] source, int amount)
    {
        String[] part = new String[Math.min(source.length, amount)];
        rng.shuffle(source);
        System.arraycopy(source, 0, part, 0, part.length);
        return part;
    }
