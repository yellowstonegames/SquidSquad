...
public final class DigitTools {
     ...
     * {@link Base#join(CharSequence, long[])}.
     ...
     * {@link Base#join(CharSequence, int[])}.
     * @param source a String containing decimal numbers separated by {@code delimiter}
     * @param delimiter a non-null, non-empty String that separates numbers in the source
     ...
     * {@link #splitFloatFromBits(String, String)}. The output is not human-readable, and uses {@link Base#SIMPLE64}.
     *
    ...
    public static String joinFloatsBits(CharSequence delimiter, float... elements) {
        ...
        Base.SIMPLE64.appendSigned(sb, BitConversion.floatToReversedIntBits(elements[0]));
        for (int i = 1; i < elements.length; i++) {
            Base.SIMPLE64.appendSigned(sb.append(delimiter), BitConversion.floatToReversedIntBits(elements[i]));
        }
        return sb.toString();
    }

    /**
     * Joins an array or varargs of floats into an Appendable CharSequence, separated by {@code delimiter}. This does
     * not write the floats conventionally; instead it converts them to their int bits and reverses the bytes of that
     * int before appending it to the Appendable. This means the float {@code 0.0f} is written as {@code 0}, most small
     * powers of 2 are 3 or 4 digits, and the longest numbers have the very least-significant bits set. This is meant to
     * be decoded by {@link #splitFloatFromBits(String, String)}. The output is not human-readable, and
     * uses {@link Base#SIMPLE64}.
     *
     * @param sb any object that implements both CharSequence and Appendable, such as {@link StringBuilder} or {@link com.github.tommyettinger.ds.CharList}
     * @param delimiter what separates the encoded float elements
     * @param elements an array or varargs of float to encode
     * @return sb, potentially after changes
     * @param <T> must implement both CharSequence and Appendable; commonly {@link StringBuilder} or {@link com.github.tommyettinger.ds.CharList}
     */
    public static <T extends CharSequence & Appendable> T appendJoinedFloatsBits(T sb, CharSequence delimiter, float... elements) {
        ...
        try {
            for (int i = 1; i < elements.length; i++) {
                sb.append(delimiter);
                Base.SIMPLE64.appendSigned(sb, BitConversion.floatToReversedIntBits(elements[i]));
            }
     ...
     * generated by {@link #joinFloatsBits(CharSequence, float...)}. The output is not human-readable, and
     * uses {@link Base#SIMPLE64}.
     *
     * @param source a String containing decimal numbers separated by {@code delimiter}
     * @param delimiter a non-null, non-empty String that separates numbers in the source
    ...
    public static float[] splitFloatFromBits(String source, String delimiter) {
        ...
        if (amount <= 0) return new float[]{BitConversion.reversedIntBitsToFloat(Base.SIMPLE64.readInt(source))};
        float[] splat = new float[amount+1];
        int dl = delimiter.length(), idx = -dl, idx2;
        for (int i = 0; i < amount; i++) {
            splat[i] = BitConversion.reversedIntBitsToFloat(Base.SIMPLE64.readInt(source, idx+dl, idx = source.indexOf(delimiter, idx+dl)));
        }
        if((idx2 = source.indexOf(delimiter, idx+dl)) < 0)
        {
            splat[amount] = BitConversion.reversedIntBitsToFloat(Base.SIMPLE64.readInt(source, idx+dl, source.length()));
        }
        else
        {
            splat[amount] = BitConversion.reversedIntBitsToFloat(Base.SIMPLE64.readInt(source, idx+dl, idx2));
        }
        return splat;
    }
