...
public class Noise {
    ...
    TAFFY_FRACTAL = 17,
    /**
     * A smooth, flowing kind of noise that allows extra configuration via {@link #setMutation(float)}, producing small
     * changes when the mutation value is similar, or large changes if it is very different. This contrasts with changes
     * to the seed, which almost always cause large changes for any difference in seed. Flan is very optimized towards
     * generating high-dimensional noise, and doesn't slow down in high dimensions as much as {@link #VALUE},
     * {@link #PERLIN}, {@link #CUBIC}, {@link #FOAM}, {@link #HONEY}, or {@link #MUTANT}.
     * This is very similar to {@link #MUTANT} and {@link #TAFFY}, but should have gradual, smooth changes, and may
     * look pillow-like if the frequency is low. This is potentially a good option if {@link #MUTANT} is too slow,
     * and you need higher-dimensional (4D and up) noise, but the appearance of {@link #TAFFY} is unsuitable.
     * <br>
     * <a href="https://i.imgur.com/3lCuYIS.png">Noise sample at left, FFT at right.</a> (This is 6D noise.)
     * <br>
     * This is meant to be used with {@link #setNoiseType(int)}.
     */
    FLAN = 18,
     ...
     * generating high-dimensional noise, and doesn't slow down in high dimensions as much as {@link #VALUE},
     * {@link #PERLIN}, {@link #CUBIC}, {@link #FOAM}, {@link #HONEY}, or {@link #MUTANT}.
     * This is very similar to {@link #MUTANT_FRACTAL} and {@link #TAFFY_FRACTAL}, but should have gradual, smooth changes, and may
     * look pillow-like if the frequency is low. This is potentially a good option if {@link #MUTANT_FRACTAL} is too slow,
     * and you need higher-dimensional (4D and up) noise, but the appearance of {@link #TAFFY_FRACTAL} is unsuitable.
     * <br>
     * <a href="https://i.imgur.com/3lCuYIS.png">Noise sample at left, FFT at right.</a> (This is 6D noise.)
     * <br>
     * This is meant to be used with {@link #setNoiseType(int)}.
     ...
     * {@link #TAFFY_FRACTAL} (17), {@link #FLAN} (18), or {@link #FLAN_FRACTAL} (19).
     ...
     * @param noiseType an int from 0 to 19 corresponding to a constant from this class for a noise type
     */
     ...
     * @return the noise type as a code, from 0 to 19 inclusive
     */
...
//        return (xd + GRADIENTS_2D[h2]) * GRADIENTS_2D[h1] + (yd + GRADIENTS_2D[h2 + 1]) * GRADIENTS_2D[h1 + 1];
    ...
    protected float pearCoord3D(int seed, int x, int y, int z, float xd, float yd, float zd) {
        ...
        return xd * GRADIENTS_3D[h1] + yd * GRADIENTS_3D[h1+1] + zd * GRADIENTS_3D[h1+2] +
                (1f - xd) * GRADIENTS_3D[h2] + (1f - yd) * GRADIENTS_3D[h2+1] + (1f - zd) * GRADIENTS_3D[h2+2];
    }
    ...
    protected float pearCoord4D(int seed, int x, int y, int z, int w, float xd, float yd, float zd, float wd) {
        ...
        return xd * GRADIENTS_4D[h1] + yd * GRADIENTS_4D[h1 + 1] + zd * GRADIENTS_4D[h1 + 2] + wd * GRADIENTS_4D[h1 + 3] +
                (1f - xd) * GRADIENTS_4D[h2] + (1f - yd) * GRADIENTS_4D[h2 + 1] + (1f - zd) * GRADIENTS_4D[h2 + 2] + (1f - wd) * GRADIENTS_4D[h2 + 3];
    }
    ...
    protected float pearCoord5D(int seed, int x, int y, int z, int w, int u,
        ...
        return xd * GRADIENTS_5D[h1] + yd * GRADIENTS_5D[h1+1] + zd * GRADIENTS_5D[h1+2] + wd * GRADIENTS_5D[h1+3] + ud * GRADIENTS_5D[h1+4] +
                (1f - xd) * GRADIENTS_5D[h2] + (1f - yd) * GRADIENTS_5D[h2+1] + (1f - zd) * GRADIENTS_5D[h2+2] + (1f - wd) * GRADIENTS_5D[h2+3] + (1f - ud) * GRADIENTS_5D[h2+4];
    }
    ...
    protected float pearCoord6D(int seed, int x, int y, int z, int w, int u, int v,
        ...
        return xd * GRADIENTS_6D[h1] + yd * GRADIENTS_6D[h1+1] + zd * GRADIENTS_6D[h1+2] +
                ...
                (1f - xd) * GRADIENTS_6D[h2] + (1f - yd) * GRADIENTS_6D[h2+1] + (1f - zd) * GRADIENTS_6D[h2+2] +
                (1f - wd) * GRADIENTS_6D[h2+3] + (1f - ud) * GRADIENTS_6D[h2+4] + (1f - vd) * GRADIENTS_6D[h2+5];
    }
    ...
    public float getConfiguredNoise(float x, float y) {
        ...
        switch (noiseType) {
            ...
            case FLAN:
                return singleFlan(seed, x, y, mutation);
            case FLAN_FRACTAL:
                switch (fractalType) {
                    case BILLOW:
                        return singleFlanFractalBillow(x, y, mutation);
                    case RIDGED_MULTI:
    ...
    public float getConfiguredNoise(float x, float y, float z) {
        ...
        switch (noiseType) {
            ...
            case FLAN:
                return singleFlan(seed, x, y, z, mutation);
            case FLAN_FRACTAL:
                switch (fractalType) {
                    case BILLOW:
                        return singleFlanFractalBillow(x, y, z, mutation);
                    case RIDGED_MULTI:
    ...
    public float getConfiguredNoise(float x, float y, float z, float w) {
        ...
        switch (noiseType) {
            ...
            case FLAN:
                return singleFlan(seed, x, y, z, w, mutation);
            case FLAN_FRACTAL:
                switch (fractalType) {
                    case BILLOW:
                        return singleFlanFractalBillow(x, y, z, w, mutation);
                    case RIDGED_MULTI:
                        return singleFlanFractalRidgedMulti(x, y, z, w, mutation);
                    default:
                        return singleFlanFractalFBM(x, y, z, w, mutation);
                }
            case PEAR:
                return singlePear(seed, x, y, z, w);
            case PEAR_FRACTAL:
                switch (fractalType) {
                    case BILLOW:
                        return singlePearFractalBillow(x, y, z, w);
                    case RIDGED_MULTI:
                        return singlePearFractalRidgedMulti(x, y, z, w);
                    default:
                        return singlePearFractalFBM(x, y, z, w);
                }
            case HONEY:
    ...
    public float getConfiguredNoise(float x, float y, float z, float w, float u) {
        ...
        switch (noiseType) {
            ...
            case FLAN:
                return singleFlan(seed, x, y, z, w, u, mutation);
            case FLAN_FRACTAL:
                switch (fractalType) {
                    case BILLOW:
                        return singleFlanFractalBillow(x, y, z, w, u, mutation);
                    case RIDGED_MULTI:
                        return singleFlanFractalRidgedMulti(x, y, z, w, u, mutation);
                    default:
                        return singleFlanFractalFBM(x, y, z, w, u, mutation);
                }
            case PEAR:
                return singlePear(seed, x, y, z, w, u);
            case PEAR_FRACTAL:
                switch (fractalType) {
                    case BILLOW:
                        return singlePearFractalBillow(x, y, z, w, u);
                    case RIDGED_MULTI:
                        return singlePearFractalRidgedMulti(x, y, z, w, u);
                    default:
                        return singlePearFractalFBM(x, y, z, w, u);
                }
            case HONEY:
    ...
    public float getConfiguredNoise(float x, float y, float z, float w, float u, float v) {
        ...
        switch (noiseType) {
            ...
            case FLAN:
                return singleFlan(seed, x, y, z, w, u, v, mutation);
            case FLAN_FRACTAL:
                switch (fractalType) {
                    case BILLOW:
                        return singleFlanFractalBillow(x, y, z, w, u, v, mutation);
                    case RIDGED_MULTI:
                        return singleFlanFractalRidgedMulti(x, y, z, w, u, v, mutation);
                    default:
                        return singleFlanFractalFBM(x, y, z, w, u, v, mutation);
                }
            case PEAR:
                return singlePear(seed, x, y, z, w, u, v);
            case PEAR_FRACTAL:
                switch (fractalType) {
                    case BILLOW:
                        return singlePearFractalBillow(x, y, z, w, u, v);
                    case RIDGED_MULTI:
                        return singlePearFractalRidgedMulti(x, y, z, w, u, v);
                    default:
                        return singlePearFractalFBM(x, y, z, w, u, v);
                }
            case HONEY:
...
//        return (((result + sign)) / (Float.MIN_VALUE - sign + (result + sharp * diff) * one) - sign - sign) - 1f;
    ...
    public float singleFlanVarargs(int seed, float... coordinates) {
        if(coordinates == null)
            return (Hasher.randomize3Float(seed) - 0.5f) * 2f;
        switch (coordinates.length) {
            case 0:
            ...
            case 1:
                return LineWobble.wobble(seed, coordinates[0]);
            case 2:
                return singleFlan(seed, coordinates[0], coordinates[1]);
            case 3:
                return singleFlan(seed, coordinates[0], coordinates[1], coordinates[2]);
            case 4:
                return singleFlan(seed, coordinates[0], coordinates[1], coordinates[2], coordinates[3]);
            case 5:
                return singleFlan(seed, coordinates[0], coordinates[1], coordinates[2], coordinates[3], coordinates[4]);
            case 6:
                return singleFlan(seed, coordinates[0], coordinates[1], coordinates[2], coordinates[3], coordinates[4],
                        coordinates[5]);
            default:
    ...
    public float singleFlan(int seed, float x, float y) {
        float result = 0.0f;
        float warp = 0.0f;
        float p = x *    0.338155f + y *   -0.941091f;
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.968979f + y *    0.247143f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.022601f + y *   -0.999745f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.751688f + y *    0.659519f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.962133f + y *   -0.272582f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.589242f + y *    0.807957f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.338155f + y *   -0.941091f
        ) + 4213f * warp) & 0x3FFF];
    ...
    public float getFlanFractal(float x, float y) {
        ...
        switch (fractalType) {
            case FBM:
                return singleFlanFractalFBM(x, y);
            case BILLOW:
                return singleFlanFractalBillow(x, y);
            case RIDGED_MULTI:
                return singleFlanFractalRidgedMulti(x, y);
            default:
                return 0;
        }
    }

    private float singleFlanFractalFBM(float x, float y) {
        int seed = this.seed;
        float sum = singleFlan(seed, x, y);
        float amp = 1, t;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += singleFlan(seed + i, x, y) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalBillow(float x, float y) {
        int seed = this.seed;
        float sum = Math.abs(singleFlan(seed, x, y)) * 2 - 1;
        float amp = 1, t;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singleFlan(++seed, x, y)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalRidgedMulti(float x, float y) {
        int seed = this.seed;
        float t;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singleFlan(seed + i, x, y));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            t = x;
            x = y * lacunarity;
    ...
    public float getFlanFractal(float x, float y, float z) {
        x *= frequency;
        y *= frequency;
        z *= frequency;
        ...
        switch (fractalType) {
            case FBM:
                return singleFlanFractalFBM(x, y, z);
            case BILLOW:
                return singleFlanFractalBillow(x, y, z);
            case RIDGED_MULTI:
                return singleFlanFractalRidgedMulti(x, y, z);
            default:
                return 0;
        }
    }

    private float singleFlanFractalFBM(float x, float y, float z) {
        int seed = this.seed;
        float sum = singleFlan(seed, x, y, z);
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += singleFlan(++seed, x, y, z) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalBillow(float x, float y, float z) {
        int seed = this.seed;
        float sum = Math.abs(singleFlan(seed, x, y, z)) * 2 - 1;
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singleFlan(++seed, x, y, z)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalRidgedMulti(float x, float y, float z) {
        int seed = this.seed;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singleFlan(seed + i, x, y, z));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            x *= lacunarity;
            y *= lacunarity;
    ...
    public float getFlan(float x, float y, float z) {
        return singleFlan(seed, x * frequency, y * frequency, z * frequency);
    }

    public float singleFlan(int seed, float x, float y, float z){
        float result = 0.0f;
        float warp = 0.0f;
        float p = x *   -0.134788f + y *   -0.728780f + z *    0.671351f;
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.892868f + y *    0.433405f + z *    0.122255f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.254056f + y *   -0.291899f + z *   -0.922090f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.047258f + y *   -0.984379f + z *    0.169604f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.520499f + y *    0.421002f + z *   -0.742858f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.862550f + y *   -0.245301f + z *    0.442532f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.563921f + y *   -0.796645f + z *   -0.217599f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.477837f + y *    0.394641f + z *    0.784812f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.301278f + y *   -0.754801f + z *   -0.582672f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.134788f + y *   -0.728780f + z *    0.671351f
        ) + 4213f * warp) & 0x3FFF];
    ...
    private float singleFlanFractalFBM(float x, float y, float z, float w) {
        int seed = this.seed;
        float sum = singleFlan(seed, x, y, z, w);
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += singleFlan(++seed, x, y, z, w) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalBillow(float x, float y, float z, float w) {
        int seed = this.seed;
        float sum = Math.abs(singleFlan(seed, x, y, z, w)) * 2 - 1;
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singleFlan(++seed, x, y, z, w)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalRidgedMulti(float x, float y, float z, float w) {
        int seed = this.seed;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singleFlan(seed + i, x, y,  z, w));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            x *= lacunarity;
            y *= lacunarity;
    ...
    public float singleFlan(int seed, float x, float y, float z, float w) {
        float result = 0.0f;
        float warp = 0.0f;
        float p = x *   -0.552927f + y *    0.821782f + z *    0.106047f + w *   -0.087756f;
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.831330f + y *    0.487350f + z *    0.256250f + w *    0.075604f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.338008f + y *   -0.048518f + z *   -0.390907f + w *   -0.854744f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.117081f + y *   -0.554527f + z *    0.800384f + w *    0.195391f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.100545f + y *    0.826306f + z *    0.020227f + w *   -0.553806f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.414709f + y *    0.317393f + z *   -0.770697f + w *    0.365110f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.708876f + y *   -0.160587f + z *    0.489766f + w *   -0.481494f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.896568f + y *   -0.360364f + z *   -0.082737f + w *    0.243842f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.422423f + y *    0.454833f + z *   -0.757905f + w *   -0.200664f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.438345f + y *    0.210921f + z *    0.323050f + w *    0.811791f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.216906f + y *   -0.536011f + z *   -0.725115f + w *   -0.373968f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.078105f + y *   -0.588459f + z *    0.556922f + w *    0.580907f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.552927f + y *    0.821782f + z *    0.106047f + w *   -0.087756f
        ) + 4213f * warp) & 0x3FFF];
    ...
    public float getFlanFractal(float x, float y, float z, float w, float u) {
        ...
        switch (fractalType) {
            case FBM:
                return singleFlanFractalFBM(x, y, z, w, u);
            case BILLOW:
                return singleFlanFractalBillow(x, y, z, w, u);
            case RIDGED_MULTI:
                return singleFlanFractalRidgedMulti(x, y, z, w, u);
            default:
                return 0;
        }
    }

    private float singleFlanFractalFBM(float x, float y, float z, float w, float u) {
        final int seed = this.seed;
        float sum = singleFlan(seed, x, y, z, w, u);
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += singleFlan(seed + i, x, y, z, w, u) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalBillow(float x, float y, float z, float w, float u) {
        final int seed = this.seed;
        float sum = Math.abs(singleFlan(seed, x, y, z, w, u)) * 2 - 1;
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singleFlan(seed + i, x, y, z, w, u)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalRidgedMulti(float x, float y, float z, float w, float u) {
        final int seed = this.seed;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singleFlan(seed + i, x, y, z, w, u));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            x *= lacunarity;
            y *= lacunarity;
    ...
    public float getFlan(float x, float y, float z, float w, float u) {
        return singleFlan(seed, x * frequency, y * frequency, z * frequency, w * frequency, u * frequency);
    }

    public float singleFlan(int seed, float x, float y, float z, float w, float u) {
        float result = 0.0f;
        float warp = 0.0f;
        float p = x *   -0.280185f + y *    0.138847f + z *   -0.225346f + w *   -0.603236f + u *    0.698243f;
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.782416f + y *    0.503937f + z *    0.317984f + w *    0.173229f + u *    0.052441f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.373987f + y *    0.070112f + z *   -0.175280f + w *   -0.428783f + u *   -0.800400f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.190161f + y *   -0.227022f + z *   -0.832369f + w *    0.451878f + u *    0.123567f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.034547f + y *   -0.683571f + z *    0.349368f + w *   -0.121155f + u *   -0.628332f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.094439f + y *    0.474781f + z *   -0.078283f + w *   -0.856326f + u *    0.161993f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.329016f + y *    0.241931f + z *   -0.731002f + w *    0.177852f + u *   -0.516936f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.602842f + y *   -0.100573f + z *    0.509392f + w *   -0.481036f + u *    0.368227f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.761390f + y *   -0.368677f + z *   -0.028543f + w *    0.433681f + u *   -0.308979f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.489606f + y *    0.763161f + z *   -0.327690f + w *   -0.059330f + u *    0.258786f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.361926f + y *    0.296335f + z *    0.412611f + w *   -0.756732f + u *   -0.195716f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.422391f + y *    0.090011f + z *    0.060031f + w *    0.287109f + u *    0.852906f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.155378f + y *   -0.383671f + z *   -0.651303f + w *   -0.583489f + u *   -0.252980f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.044144f + y *   -0.526774f + z *    0.512661f + w *    0.404070f + u *    0.542647f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.335082f + y *    0.913193f + z *    0.165805f + w *   -0.112036f + u *   -0.117281f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.280185f + y *    0.138847f + z *   -0.225346f + w *   -0.603236f + u *    0.698243f
        ) + 4213f * warp) & 0x3FFF];
    ...
    public float getFlanFractal(float x, float y, float z, float w, float u, float v) {
        ...
        switch (fractalType) {
            case FBM:
                return singleFlanFractalFBM(x, y, z, w, u, v);
            case BILLOW:
                return singleFlanFractalBillow(x, y, z, w, u, v);
            case RIDGED_MULTI:
                return singleFlanFractalRidgedMulti(x, y, z, w, u, v);
            default:
                return 0;
        }
    }

    private float singleFlanFractalFBM(float x, float y, float z, float w, float u, float v) {
        int seed = this.seed;
        float sum = singleFlan(seed, x, y, z, w, u, v);
        float amp = 1;
    ...
    private float singleFlanFractalBillow(float x, float y, float z, float w, float u, float v) {
        int seed = this.seed;
        float sum = Math.abs(singleFlan(seed, x, y, z, w, u, v)) * 2 - 1;
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singleFlan(++seed, x, y, z, w, u, v)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalRidgedMulti(float x, float y, float z, float w, float u, float v) {
        int seed = this.seed;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singleFlan(seed + i, x, y, z, w, u, v));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            x *= lacunarity;
            y *= lacunarity;
    ...
    public float getFlan(float x, float y, float z, float w, float u, float v) {
        return singleFlan(seed, x * frequency, y * frequency, z * frequency, w * frequency, u * frequency, v * frequency);
    }

    public float singleFlan(int seed, float x, float y, float z, float w, float u, float v) {
        float result = 0.0f;
        float warp = 0.0f;
        float p = x *   -0.488205f + y *    0.047898f + z *   -0.800964f + w *    0.335998f + u *    0.065270f + v *   -0.025754f;
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.742607f + y *    0.506572f + z *    0.349735f + w *    0.228055f + u *    0.126782f + v *    0.039027f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.390037f + y *    0.137250f + z *   -0.057170f + w *   -0.239957f + u *   -0.443276f + v *   -0.756104f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.238552f + y *   -0.090663f + z *   -0.430795f + w *    0.796370f + u *    0.325958f + v *    0.093996f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.118656f + y *   -0.365639f + z *    0.643475f + w *    0.136786f + u *   -0.198540f + v *   -0.616474f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.006903f + y *   -0.726423f + z *    0.134316f + w *   -0.262036f + u *    0.605061f + v *    0.139498f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.135659f + y *    0.503046f + z *   -0.184694f + w *    0.672615f + u *    0.020485f + v *   -0.491527f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.266058f + y *    0.189797f + z *   -0.677159f + w *    0.079268f + u *   -0.610930f + v *    0.234790f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.507432f + y *   -0.056953f + z *    0.497077f + w *   -0.449845f + u *    0.283549f + v *   -0.457627f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.656018f + y *   -0.299563f + z *    0.038421f + w *    0.544970f + u *   -0.289956f + v *    0.312025f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.739908f + y *   -0.479545f + z *   -0.217896f + w *    0.018308f + u *    0.362573f + v *   -0.208084f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.401912f + y *    0.398776f + z *    0.709811f + w *   -0.313952f + u *   -0.044616f + v *    0.273966f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.330485f + y *    0.209813f + z *    0.231431f + w *    0.395297f + u *   -0.773509f + v *   -0.196526f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.401928f + y *   -0.003139f + z *   -0.139573f + w *   -0.045910f + u *    0.256711f + v *    0.866577f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.112254f + y *   -0.277806f + z *   -0.544366f + w *   -0.615816f + u *   -0.451087f + v *   -0.176575f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.020976f + y *   -0.500020f + z *    0.499329f + w *    0.323696f + u *    0.335892f + v *    0.531609f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.202658f + y *    0.925504f + z *    0.185839f + w *   -0.107272f + u *   -0.204505f + v *   -0.120441f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.199542f + y *    0.205565f + z *   -0.141493f + w *   -0.458425f + u *    0.604759f + v *    0.567468f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.488205f + y *    0.047898f + z *   -0.800964f + w *    0.335998f + u *    0.065270f + v *   -0.025754f
        ) + 4213f * warp) & 0x3FFF];
    ...
    private float singleFlanFractalFBM(float x, float y, float z, float w, float u, float v, float m) {
        int seed = this.seed;
        float sum = singleFlan(seed, x, y, z, w, u, v, m);
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += singleFlan(++seed, x, y, z, w, u, v, m) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalBillow(float x, float y, float z, float w, float u, float v, float m) {
        int seed = this.seed;
        float sum = Math.abs(singleFlan(seed, x, y, z, w, u, v, m)) * 2 - 1;
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singleFlan(++seed, x, y, z, w, u, v, m)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    private float singleFlanFractalRidgedMulti(float x, float y, float z, float w, float u, float v, float m) {
        int seed = this.seed;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singleFlan(seed + i, x, y, z, w, u, v, m));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            x *= lacunarity;
            y *= lacunarity;
    ...
    public float singleFlan(int seed, float x, float y, float z, float w, float u, float v, float m) {
        float result = 0.0f;
        float warp = 0.0f;
        float p = x *   -0.321553f + y *   -0.037168f + z *    0.401510f + w *    0.001364f + u *   -0.233422f + v *   -0.493706f + m *    0.660135f;
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.709413f + y *    0.503254f + z *    0.366828f + w *    0.261281f + u *    0.173625f + v *    0.097803f + m *    0.030462f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.396568f + y *    0.178680f + z *    0.016151f + w *   -0.129342f + u *   -0.276343f + v *   -0.447323f + m *   -0.719273f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.287464f + y *   -0.008039f + z *   -0.277217f + w *   -0.670561f + u *    0.558483f + v *    0.269223f + m *    0.080089f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.148021f + y *   -0.179947f + z *   -0.740548f + w *    0.281252f + u *    0.017834f + v *   -0.211896f + m *   -0.522471f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.071597f + y *   -0.498519f + z *    0.392035f + w *   -0.058221f + u *   -0.514118f + v *    0.551464f + m *    0.144368f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.027417f + y *    0.788664f + z *    0.040754f + w *   -0.382342f + u *    0.273604f + v *   -0.051324f + m *   -0.389766f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.115646f + y *    0.345636f + z *   -0.197495f + w *    0.370802f + u *   -0.090368f + v *   -0.810560f + m *    0.159652f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.206954f + y *    0.144169f + z *   -0.589104f + w *    0.023314f + u *   -0.682257f + v *    0.087182f + m *   -0.340180f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.437605f + y *   -0.027884f + z *    0.480388f + w *   -0.417873f + u *    0.226572f + v *   -0.516522f + m *    0.290179f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.552858f + y *   -0.229276f + z *    0.087848f + w *    0.607720f + u *   -0.249013f + v *    0.295458f + m *   -0.339760f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.684271f + y *   -0.383685f + z *   -0.154071f + w *    0.086835f + u *    0.512454f + v *   -0.174389f + m *    0.245477f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.489835f + y *    0.605012f + z *   -0.423652f + w *   -0.178203f + u *    0.043088f + v *    0.382107f + m *   -0.186876f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.358596f + y *    0.294074f + z *    0.354523f + w *    0.690339f + u *   -0.311031f + v *   -0.034520f + m *    0.291107f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.312452f + y *    0.149939f + z *    0.117127f + w *    0.190017f + u *    0.389454f + v *   -0.798947f + m *   -0.200192f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *    0.355448f + y *   -0.069265f + z *   -0.276392f + w *   -0.286034f + u *   -0.113743f + v *    0.215912f + m *    0.806905f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *    0.078776f + y *   -0.195515f + z *   -0.427684f + w *   -0.603683f + u *   -0.532564f + v *   -0.331567f + m *   -0.121083f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.003167f + y *   -0.491935f + z *    0.502622f + w *    0.276870f + u *    0.228260f + v *    0.298509f + m *    0.536184f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.121386f + y *    0.908116f + z *    0.182218f + w *   -0.092530f + u *   -0.229441f + v *   -0.232000f + m *   -0.111165f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.151548f + y *    0.260593f + z *   -0.087992f + w *   -0.380215f + u *    0.570837f + v *    0.422059f + m *    0.502821f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed >>> 18) + (int) ((p =
                x *   -0.459652f + y *    0.191641f + z *   -0.648315f + w *    0.553556f + u *    0.143091f + v *   -0.035948f + m *   -0.059078f
        ) + 4213f * warp) & 0x3FFF];
        result += warp = TrigTools.sin(p - warp) * TrigTools.SIN_TABLE[(seed ^= (seed << 21 | seed >>> 11) + 0x9E3779B9) + (int) ((p =
                x *   -0.321553f + y *   -0.037168f + z *    0.401510f + w *    0.001364f + u *   -0.233422f + v *   -0.493706f + m *    0.660135f
        ) + 4213f * warp) & 0x3FFF];
...
//    public float minBound = -1f, maxBound = 1f;
    ...
    public float singlePear(int seed, float x, float y) {
        ...
        float e = (float)Math.pow(1.707f, n * 16f * sharpness);
        return (e - 1f) / (e + 1f);
...
//        return TrigTools.sinTurns(0x1p-3f *
    ...
    public float singlePear(int seed, float x, float y, float z) {
        ...
        float e = (float)Math.pow(1.707f, n * 11f * sharpness);
        return (e - 1f) / (e + 1f);

//        return (0x1p-4f *
//                29f * n);
    }
    private float singlePearFractalFBM(float x, float y, float z, float w) {
        int seed = this.seed;
        float sum = singlePear(seed, x, y, z, w);
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += singlePear(seed + i, x, y, z, w) * amp;
        }

        return sum * fractalBounding;
    }
    private float singlePearFractalRidgedMulti(float x, float y, float z, float w) {
        int seed = this.seed;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singlePear(seed + i, x, y, z, w));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            x *= lacunarity;
            y *= lacunarity;
    ...
    private float singlePearFractalBillow(float x, float y, float z, float w) {
        int seed = this.seed;
        float sum = Math.abs(singlePear(seed, x, y, z, w)) * 2 - 1;
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singlePear(seed + i, x, y, z, w)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    public float getPear(float x, float y, float z, float w) {
        return singlePear(seed, x * frequency, y * frequency, z * frequency, w * frequency);
    }

    public float singlePear(int seed, float x, float y, float z, float w) {
        float skew = (x + y + z + w) * F4f;
        int i = fastFloor(x + skew);
        int j = fastFloor(y + skew);
        int k = fastFloor(z + skew);
        ...
        float t0 = 0.7500f - x0 * x0 - y0 * y0 - z0 * z0 - w0 * w0;
        if(t0 > 0) {
            t0 *= t0;
            t0 *= t0 * pearCoord4D(seed, i, j, k, l, x0, y0, z0, w0);
        }
        else t0 = 0;
        float t1 = 0.7500f - x1 * x1 - y1 * y1 - z1 * z1 - w1 * w1;
        if (t1 > 0) {
            t1 *= t1;
            t1 *= t1 * pearCoord4D(seed, i + i1, j + j1, k + k1, l + l1, x1, y1, z1, w1);
        }
        else t1 = 0;
        float t2 = 0.7500f - x2 * x2 - y2 * y2 - z2 * z2 - w2 * w2;
        if (t2 > 0) {
            t2 *= t2;
            t2 *= t2 * pearCoord4D(seed, i + i2, j + j2, k + k2, l + l2, x2, y2, z2, w2);
        }
        else t2 = 0;
        float t3 = 0.7500f - x3 * x3 - y3 * y3 - z3 * z3 - w3 * w3;
        if (t3 > 0) {
            t3 *= t3;
            t3 *= t3 * pearCoord4D(seed, i + i3, j + j3, k + k3, l + l3, x3, y3, z3, w3);
        }
        else t3 = 0;
        float t4 = 0.7500f - x4 * x4 - y4 * y4 - z4 * z4 - w4 * w4;
        if (t4 > 0) {
            t4 *= t4;
            t4 *= t4 * pearCoord4D(seed, i + 1, j + 1, k + 1, l + 1, x4, y4, z4, w4);
        }
        else t4 = 0;

        float e = (t0 + t1 + t2 + t3 + t4);
        e = (float)Math.pow(1.707f, e * 9f * sharpness);
        return (e - 1f) / (e + 1f);
//        return (t0 + t1 + t2 + t3 + t4) * 0x1p-4f * 14.7279f;
    }


    private float singlePearFractalFBM(float x, float y, float z, float w, float u) {
        int seed = this.seed;
        float sum = singlePear(seed, x, y, z, w, u);
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += singlePear(seed + i, x, y, z, w, u) * amp;
        }

        return sum * fractalBounding;
    }
    private float singlePearFractalRidgedMulti(float x, float y, float z, float w, float u) {
        int seed = this.seed;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singlePear(seed + i, x, y, z, w, u));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            x *= lacunarity;
            y *= lacunarity;
    ...
    private float singlePearFractalBillow(float x, float y, float z, float w, float u) {
        int seed = this.seed;
        float sum = Math.abs(singlePear(seed, x, y, z, w, u)) * 2 - 1;
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singlePear(seed + i, x, y, z, w, u)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    public float singlePear(int seed, float x, float y, float z, float w, float u) {
        float n0, n1, n2, n3, n4, n5;
        float t = (x + y + z + w + u) * F5;
        int i = fastFloor(x + t);
        int j = fastFloor(y + t);
        ...
        t = 0.800f - x0 * x0 - y0 * y0 - z0 * z0 - w0 * w0 - u0 * u0;
        if (t < 0) n0 = 0;
        else
        {
            t *= t;
            n0 = t * t * pearCoord5D(seed, i, j, k, l, h, x0, y0, z0, w0, u0);
        }

        t = 0.800f - x1 * x1 - y1 * y1 - z1 * z1 - w1 * w1 - u1 * u1;
        if (t < 0) n1 = 0;
        else
        {
            t *= t;
            n1 = t * t * pearCoord5D(seed, i + i1, j + j1, k + k1, l + l1, h + h1, x1, y1, z1, w1, u1);
        }

        t = 0.800f - x2 * x2 - y2 * y2 - z2 * z2 - w2 * w2 - u2 * u2;
        if (t < 0) n2 = 0;
        else
        {
            t *= t;
            n2 = t * t * pearCoord5D(seed, i + i2, j + j2, k + k2, l + l2, h + h2, x2, y2, z2, w2, u2);
        }

        t = 0.800f - x3 * x3 - y3 * y3 - z3 * z3 - w3 * w3 - u3 * u3;
        if (t < 0) n3 = 0;
        else
        {
            t *= t;
            n3 = t * t * pearCoord5D(seed, i + i3, j + j3, k + k3, l + l3, h + h3, x3, y3, z3, w3, u3);
        }

        t = 0.800f - x4 * x4 - y4 * y4 - z4 * z4 - w4 * w4 - u4 * u4;
        if (t < 0) n4 = 0;
        else
        {
            t *= t;
            n4 = t * t * pearCoord5D(seed, i + i4, j + j4, k + k4, l + l4, h + h4, x4, y4, z4, w4, u4);
        }

        t = 0.800f - x5 * x5 - y5 * y5 - z5 * z5 - w5 * w5 - u5 * u5;
        if (t < 0) n5 = 0;
        else
        {
            t *= t;
            n5 = t * t * pearCoord5D(seed, i + 1, j + 1, k + 1, l + 1, h + 1, x5, y5, z5, w5, u5);
        }

        float e = (n0 + n1 + n2 + n3 + n4 + n5);
        e = (float)Math.pow(1.707f, e * 10f * sharpness);
        return (e - 1f) / (e + 1f);
//        return  (n0 + n1 + n2 + n3 + n4 + n5) * 10f;
    }

    private float singlePearFractalFBM(float x, float y, float z, float w, float u, float v) {
        int seed = this.seed;
        float sum = singlePear(seed, x, y, z, w, u, v);
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += singlePear(seed + i, x, y, z, w, u, v) * amp;
        }

        return sum * fractalBounding;
    }
    private float singlePearFractalRidgedMulti(float x, float y, float z, float w, float u, float v) {
        int seed = this.seed;
        float sum = 0f, exp = 2f, correction = 0f, spike;
        for (int i = 0; i < octaves; i++) {
            spike = 1f - Math.abs(singlePear(seed + i, x, y, z, w, u, v));
            correction += (exp *= 0.5f);
            sum += spike * exp;
            x *= lacunarity;
            y *= lacunarity;
    ...
    private float singlePearFractalBillow(float x, float y, float z, float w, float u, float v) {
        int seed = this.seed;
        float sum = Math.abs(singlePear(seed, x, y, z, w, u, v)) * 2 - 1;
        float amp = 1;
        ...
        for (int i = 1; i < octaves; i++) {
            ...
            amp *= gain;
            sum += (Math.abs(singlePear(seed + i, x, y, z, w, u, v)) * 2 - 1) * amp;
        }

        return sum * fractalBounding;
    }

    public float singlePear(int seed, float x, float y, float z, float w, float u, float v) {
        float n0, n1, n2, n3, n4, n5, n6;
        float t = (x + y + z + w + u + v) * F6;
        int i = fastFloor(x + t);
        int j = fastFloor(y + t);
        ...
        n0 = 0.75000f - x0 * x0 - y0 * y0 - z0 * z0 - w0 * w0 - u0 * u0 - v0 * v0;
        if (n0 <= 0f) n0 = 0f;
        else
        {
            n0 *= n0;
            n0 *= n0 * pearCoord6D(seed, i, j, k, l, h, g, x0, y0, z0, w0, u0, v0);
        }

        n1 = 0.75000f - x1 * x1 - y1 * y1 - z1 * z1 - w1 * w1 - u1 * u1 - v1 * v1;
        if (n1 <= 0f) n1 = 0f;
        else
        {
            n1 *= n1;
            n1 *= n1 * pearCoord6D(seed, i + i1, j + j1, k + k1, l + l1, h + h1, g + g1, x1, y1, z1, w1, u1, v1);
        }

        n2 = 0.75000f - x2 * x2 - y2 * y2 - z2 * z2 - w2 * w2 - u2 * u2 - v2 * v2;
        if (n2 <= 0f) n2 = 0f;
        else
        {
            n2 *= n2;
            n2 *= n2 * pearCoord6D(seed, i + i2, j + j2, k + k2, l + l2, h + h2, g + g2, x2, y2, z2, w2, u2, v2);
        }

        n3 = 0.75000f - x3 * x3 - y3 * y3 - z3 * z3 - w3 * w3 - u3 * u3 - v3 * v3;
        if (n3 <= 0f) n3 = 0f;
        else
        {
            n3 *= n3;
            n3 *= n3 * pearCoord6D(seed, i + i3, j + j3, k + k3, l + l3, h + h3, g + g3, x3, y3, z3, w3, u3, v3);
        }

        n4 = 0.75000f - x4 * x4 - y4 * y4 - z4 * z4 - w4 * w4 - u4 * u4 - v4 * v4;
        if (n4 <= 0f) n4 = 0f;
        else
        {
            n4 *= n4;
            n4 *= n4 * pearCoord6D(seed, i + i4, j + j4, k + k4, l + l4, h + h4, g + g4, x4, y4, z4, w4, u4, v4);
        }

        n5 = 0.75000f - x5 * x5 - y5 * y5 - z5 * z5 - w5 * w5 - u5 * u5 - v5 * v5;
        if (n5 <= 0f) n5 = 0f;
        else
        {
            n5 *= n5;
            n5 *= n5 * pearCoord6D(seed, i + i5, j + j5, k + k5, l + l5, h + h5, g + g5, x5, y5, z5, w5, u5, v5);
        }

        n6 = 0.75000f - x6 * x6 - y6 * y6 - z6 * z6 - w6 * w6 - u6 * u6 - v6 * v6;
        if (n6 <= 0f) n6 = 0f;
        else
        {
            n6 *= n6;
            n6 *= n6 * pearCoord6D(seed, i + 1, j + 1, k + 1, l + 1, h + 1, g + 1, x6, y6, z6, w6, u6, v6);
        }

        float e = (n0 + n1 + n2 + n3 + n4 + n5 + n6);
        e = (float)Math.pow(1.707f, e * 55f * sharpness);
        return (e - 1f) / (e + 1f);
//        return  (n0 + n1 + n2 + n3 + n4 + n5 + n6) * 7.499f;
    }
