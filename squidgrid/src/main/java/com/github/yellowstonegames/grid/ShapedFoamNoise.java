 ...
 * <br>
 * This is normally a smooth type of noise that can work well using fewer octaves than simplex noise or value noise.
...
public class ShapedFoamNoise implements INoise {
    ...
    /**
     * The shape parameter of the spline this uses to smooth value noise results. This defaults to 2, which makes the
     * shape similar to a cubic Hermite spline function, but can be 1 to linear-interpolate, or between 0 and 1 to make
     * noise the opposite of smooth.
     */
    protected float shape = 2f;

    public static final ShapedFoamNoise instance = new ShapedFoamNoise();
    ...
    /**
     * The smoothing function applied to an x-value between 0 and 1, both inclusive. This can use the {@link #shape}
     * value to determine how it works, but doesn't have to in subclasses. This defaults to using
     * {@link com.github.tommyettinger.digital.MathTools#barronSpline(float, float, float)} with 0.5f for turning, and
     * the shape of this object for shape.
     * @param x between 0 and 1, both inclusive
     * @return a float between 0 and 1, both inclusive
     */
    protected float spline(final float x) {
        final float d = 0.5f - x;
        final int f = BitConversion.floatToIntBits(d) >> 31, n = f | 1;
        return (0.5f * n - f) * (x + f) / (0x1.0p-126f - f + (x + shape * d) * n) - f;
    }
     ...
     */
    public float valueNoise(float x, float y, final long seed) {
     ...
     * @return noise between 0 and 1
     */
    public float valueNoise(float x, float y, float z, final long seed) {
     ...
     * @return noise between 0 and 1
     */
    public float valueNoise(float x, float y, float z, float w, final long seed) {
     ...
     * @return noise between 0 and 1
     */
    public float valueNoise(float x, float y, float z, float w, float u, long seed) {
    ...
    public float getNoiseWithSeed(float x, float y, float z, float w, float u, float v, float m, long seed) {
        ...
        float vin = p4;
        float min = p5;
        final float a = valueNoise(xin, yin, zin, win, uin, vin, min, seed);
        xin = p2;
        yin = p3;
        zin = p0;
        win = p4;
        ...
        vin = p5;
        min = p7;
        final float b = valueNoise(xin + a, yin, zin, win, uin, vin, min, seed + 0x9A827999FCEF3243L);
        xin = p1;
        yin = p2;
        zin = p4;
        win = p3;
        ...
        vin = p7;
        min = p6;
        final float c = valueNoise(xin + b, yin, zin, win, uin, vin, min, seed + 0x3504F333F9DE6486L);
        xin = p7;
        yin = p0;
        zin = p2;
        win = p5;
        ...
        vin = p6;
        min = p1;
        final float d = valueNoise(xin + c, yin, zin, win, uin, vin, min, seed + 0xCF876CCDF6CD96C9L);
        xin = p3;
        yin = p1;
        zin = p5;
        win = p6;
        ...
        vin = p0;
        min = p2;
        final float e = valueNoise(xin + d, yin, zin, win, uin, vin, min, seed + 0x6A09E667F3BCC90CL);
        xin = p4;
        yin = p7;
        zin = p6;
        win = p2;
        ...
        vin = p1;
        min = p3;
        final float f = valueNoise(xin + e, yin, zin, win, uin, vin, min, seed + 0x48C6001F0ABFB4FL);
        xin = p5;
        yin = p4;
        zin = p7;
        win = p1;
        ...
        vin = p3;
        min = p0;
        final float g = valueNoise(xin + f, yin, zin, win, uin, vin, min, seed + 0x9F0ED99BED9B2D92L);
        xin = p6;
        yin = p5;
        zin = p1;
        win = p0;
    ...
    public float valueNoise(float x, float y, float z, float w, float u, float v, float m, long seed) {
     ...
     *
     * @return the maximum supported dimension, which is 7 inclusive
     */
    @Override
