...
public class QuasiRandomTools {
    ...
    /**
     * Fills a buffer (or creates one) with quasi-random floats between 0 (inclusive) and 1 (exclusive). Indices in the
     * buffer from {@code position} (inclusive) to {@code position + dimensions} (exclusive) will be assigned what
     * should almost always be distinct values from a quasi-random sequence described by Martin Roberts. The
     * {@code index} parameter is the index in that sequence, and should be greater than 0; typically as you need more
     ...
     * @param index the index of the Coord in a Roberts sequence; should be greater than 0, but not required to be
     * @return buffer, after modifications, or a newly-created float array if buffer was null or too-small
     */
    public static float[] robertsND(float[] buffer, int position, int dimensions, int index) {
        dimensions = Math.min(Math.max(dimensions, 1), 50);
        if(buffer == null || buffer.length - position < dimensions)
        {
            position = 0;
    ...
    public static final float[][] goldenFloat = {
            {0.6180339887498949f, },
