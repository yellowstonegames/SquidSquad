...
public class CoordFloatOrderedMap extends ObjectFloatOrderedMap<Coord> implements ISerializersNeeded {
     ...
     * Values are parsed with {@link Base#readLong(CharSequence, int, int)} using {@link Base#BASE10}.
     ...
     * Keys are parsed with {@link Coord#COORD_PARSER}.
     * Values are parsed with {@link Base#readFloat(CharSequence, int, int)} using {@link Base#BASE10}.
     * Extra brackets inside the given range
     * of characters will ruin the parsing, so increase offset by 1 and
     * reduce length by 2 if the original String had brackets added to it.
     *
