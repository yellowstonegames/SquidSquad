 ...
 * <br>
 * A potential use for this class is to sort Coord positions by long values, such as the {@code long[][]} maps
 * produced by {@link FOV}. Since those have a logical start point for iteration wherever a light source is, and a light
 * source has the highest value, you could fill a CoordLongOrderedMap with all Coords you wanted to iterate through,
 * associated with their values in the {@code long[][]}, and call {@link #sortByValue(LongComparator)} to change the
 * keys' iteration order. There is code for this in {@link #fromArray2D(long[][], long, long)}.
 */
