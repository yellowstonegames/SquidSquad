...
public class Region implements Collection<Coord> {
    ...
    /**
     * Takes another Region, called other, with potentially different size and inserts its "on" cells into this
     * Region at the given x,y offset, allowing negative x and/or y to put only part of other in this.
     * <br>
     * This allocates a long array internally; to avoid that, you can use {@link #insert(int, int, Region, Region)}.
     *
    ...
    /**
     * Takes the "on" cells in this Region and expands them by {@code amount} cells in the 4 orthogonal directions,
    ...
    /**
     * Takes the "on" cells in this Region and produces {@code amount} Regions, each one expanded by 1 cell in
    ...
    public Region[] expandSeries(int amount) {
        ...
        if (amount <= 0) {
            ...
            for (int i = 0; i < amount; i++) {
                result[i] = new Region(work.expand(temp));
            }
        }
        return result;
    }
    ...
    public Region retract(Region temp) {
        if(width <= 2 || height <= 2) {
            ...
            // Equivalent to the unusual arraycopy call in zero-argument retract().
            for (int a = 0; a < ySections; a++) {
                self[a] = 0L;
                self[a + width - 1] = 0L;
            }
     ...
     * making each "on" cell take up a 3x3 square-shaped area that may overlap with other "on" cells (which is just a
     * normal "on" cell then).
     * <br>
     * This operates in bulk on up to 64 cells at a time.
     * This overload allocates an array the same size as the one used internally in this Region.
     * To avoid allocating anything, you can reuse a Region the same size as this one and pass it to
     * {@link #expand8way(Region)}.
     *
     * @return this for chaining
     */
    ...
    public Region expand8way(Region temp) {
        ...
        if (width > 0 && ySections != 0) {
            ...
            final long[] copy = temp.data;
            for (int a = 0; a < ySections; a++) {
                self[a] |= (copy[a] << 1) | (copy[a] >>> 1)
                        | copy[a + ySections] | (copy[a + ySections] << 1) | (copy[a + ySections] >>> 1);
                self[(width - 1) * ySections + a] |= (copy[(width - 1) * ySections + a] << 1) | (copy[(width - 1) * ySections + a] >>> 1)
                        | copy[(width - 2) * ySections + a] | (copy[(width - 2) * ySections + a] << 1) | (copy[(width - 2) * ySections + a] >>> 1);

                for (int i = ySections + a; i < (width - 1) * ySections; i += ySections) {
                    self[i] |= (copy[i] << 1) | (copy[i] >>> 1)
    ...
    /**
     * Takes the "on" cells in this Region and expands them by {@code amount} cells in the 8 orthogonal and diagonal
     * directions, making each "on" cell take up a square-shaped area that may overlap with other "on" cells (which is
     * just a normal "on" cell then).
     * <br>
     * This operates in bulk on up to 64 cells at a time.
     * This overload allocates an array the same size as the one used internally in this Region.
     * To avoid allocating anything, you can reuse a Region the same size as this one and pass it to
     * {@link #expand8way(int, Region)}.
     *
     * @param amount how thick the bordering area should be
     * @return this for chaining
     */
    ...
    {
        return expand8way(amount, null);
    }
    /**
     * Takes the "on" cells in this Region and expands them by {@code amount} cells in the 8 orthogonal and diagonal
     * directions, making each "on" cell take up a square-shaped area that may overlap with other "on" cells (which
     * is just a normal "on" cell then).
     * <br>
     * This operates in bulk on up to 64 cells at a time.
     * This overload takes a temporary Region {@code temp} that should be the same size as this Region. If temp is
     * non-null, it will be cleared and receive the contents of this Region before this method call.
    ...
    /**
     * Takes the "on" cells in this Region and produces {@code amount} Regions, each one expanded by 1 cell in
     * the 8 orthogonal and diagonal directions relative to the previous Region, making each "on" cell take up a
     * square-shaped area that may overlap with other "on" cells (which is just a normal "on" cell then).
     * This returns an array of Regions with progressively greater expansions, and does not modify this Region.
     * <br>
     * This operates in bulk on up to 64 cells at a time.
     * This method allocates multiple copied Regions and returns most of them in an array.
     *
    ...
    public Region[] expandSeries8way(int amount) {
        ...
        if (amount <= 0) {
            ...
            Region work = new Region(this), temp = new Region(this);
            for (int i = 0; i < amount; i++) {
                result[i] = new Region(work.expand8way(temp));
            }
        }
        return result;
    }
    ...
    /**
     * Returns a new ObjectList of Region where each item is a progressively more and more {@link #expand8way()}-ed
     * copy of this Region, until just before the entire Region is "on" cells.
     * @return a new ObjectList of copies of this Region, each expanded more and more until expansion can't add cells
     */
    public ObjectList<Region> expandSeriesToLimit8way()
    {
        ObjectList<Region> regions = new ObjectList<>();
        Region work = new Region(this), temp = new Region(this);
        while (work.size() != work.expand8way(temp).size()) {
            regions.add(new Region(work));
        }
        return regions;
    }
    ...
    public Region flood(Region bounds, Region buffer) {
        if (width > 0 && ySections > 0 && bounds != null && bounds.width >= 2 && bounds.ySections > 0) {
            ...
            final long[] self = this.data;
            for (int a = 0; a < ySections && a < bounds.ySections; a++) {
                self[a] |= (copy[a] | (copy[a] << 1) | (copy[a] >>> 1) | copy[a + ySections]) & bounds.data[a];
                self[(width - 1) * ySections + a] |= (copy[(width - 1) * ySections + a] | (copy[(width - 1) * ySections + a] << 1)
                        | (copy[(width - 1) * ySections + a] >>> 1) | copy[(width - 2) * ySections + a]) & bounds.data[(width - 1) * bounds.ySections + a];

                for (int i = ySections + a, j = bounds.ySections + a; i < (width - 1) * ySections &&
                        j < (bounds.width - 1) * bounds.ySections; i += ySections, j += bounds.ySections) {
                    self[i] |= (copy[i] | (copy[i] << 1) | (copy[i] >>> 1) | copy[i - ySections] | copy[i + ySections]) & bounds.data[j];
                }

                if (a > 0) {
                    for (int i = ySections + a, j = bounds.ySections + a; i < (width - 1) * ySections && j < (bounds.width - 1) * bounds.ySections;
                         i += ySections, j += bounds.ySections) {
                        self[i] |= (copy[i] | ((copy[i - 1] & 0x8000000000000000L) >>> 63)) & bounds.data[j];
                    }
                }
                ...
                if (a < ySections - 1 && a < bounds.ySections - 1) {
                    for (int i = ySections + a, j = bounds.ySections + a;
                         i < (width - 1) * ySections && j < (bounds.width - 1) * bounds.ySections; i += ySections, j += bounds.ySections) {
                        self[i] |= (copy[i] | ((copy[i + 1] & 1L) << 63)) & bounds.data[j];
                    }
                }
            }
            if (yEndMask != -1 && bounds.yEndMask != -1) {
        ...
        if (amount <= 0) {
            ...
            for (int i = 0; i < amount; i++) {
                if (done) {
                    ...
                    regions[i] = new Region(temp.flood(bounds, buffer));
                    if (ct == (ct2 = temp.size()))
                        done = true;
                    else
                        ct = ct2;
    ...
    /**
     * A selective flood-fill that modifies this Region so it adds adjacent cells where {@code noise.getNoise(x, y)}
     * evaluates between {@code lowerBound} (inclusive) and {@code upperBound} (exclusive), if it can while staying
     * inside the "on" cells of {@code bounds}. This Region acts
     * as the initial state, and often contains just one cell before this is called. This method is useful for imitating
     * the movement of fluids like water or smoke within some boundaries, stepping only to adjacent cells instead of how
     * {@link #spill(Region, int, EnhancedRandom)} fills a whole volume, or how {@link #splash(Region, EnhancedRandom)}
     * only steps to one random cell. Because this requests 2D noise, unless the noise has its seed change between calls
     * (making each call totally and randomly different), some areas will usually not be possible to fill with typical
     * bounds (ones that don't just accept all noise as in-bounds). The result from each call to getNoise() should be
    ...
    public Region stir(Region bounds, INoise noise, float lowerBound, float upperBound) {
        return stir(bounds, noise, lowerBound, upperBound, null);
    }
