 ...
 *     Region until it reaches {@code size}, {@link #deteriorate(Random, double)} randomly removes
 *     points but stops when the fraction of cells remaining is equal to {@code preservation}, {@link #fray(double)}
 *     acts like {@link #deteriorate(Random, double)} but only affects the surface (what {@link #surface()}
 ...
 *     to get an approximate fraction of well-separated "on" cells from a Region. {@link #fray(double)} also has a quasi-random version that
 ...
 *     {@link #separatedBlue(double)} if you don't want a random aspect),</li>
...
public class Region implements Collection<Coord>, Serializable {
    ...
    /**
     * Constructs this Region using a double[][] (typically one generated by
    ...
    /**
     * Constructs this Region using a double[][] that only stores two relevant states: an "on" state for values
     ...
     * so on.
     * @param map a double[][]; depending on scale, the Region may have different width and height
     * @param lowerBound lower inclusive; any double lower than this will be off, any equal to or greater than this,
     *                   but less than upper, will be on
     * @param upperBound upper exclusive; any double greater than or equal to this this will be off, any doubles both
     *                   less than this and equal to or greater than lower will be on
     ...
     */
    public Region(final double[][] map, final double lowerBound, final double upperBound, int scale)
    {
     ...
     * @return this for chaining
     */
    public Region refill(final double[][] map, final double lowerBound, final double upperBound, int scale) {
        scale = Math.min(63, Math.max(1, scale));
     ...
     * @param height the maximum height for the Region
     */
    public Region(final Random random, final double fraction, final int width, final int height)
    {
        this.width = width;
        this.height = height;
     ...
     * @return this for chaining
     */
    public Region refill(final Random random, final double fraction, final int width, final int height) {
        if (random != null){
            int bitCount = (int) (fraction * 64);
            if(this.width == width && this.height == height) {
                for (int i = 0; i < width * ySections; i++) {
     ...
     * running {@link #surface()} on a copy of this Region, running {@link #separatedRegionBlue(double)} on that
     ...
     * {@link #surface()} on a copy of this Region, running {@link #deteriorate(Random, double)} on
     ...
     * @return this for chaining
     */
    public Region fray(Random random, double fractionKept)
    {
        Region cpy = new Region(this).retract();
    ...
    */
     ...
     * <br>
     * Internally, this is a thin wrapper around {@link #separatedRegionBlue(double, int)}, and won't be more efficient
     * than that method.
     ...
     * @return this, after modifications, for chaining
     */
    public Region separatedRegionBlue(double fraction)
    {
     ...
     * @return this, after modifications, for chaining
     */
    public Region separatedRegionBlue(double fraction, int limit)
    {
        if(fraction <= 0)
            return empty();
        if(fraction > 0xFFp-8)
     ...
     * @return a randomly modified change to this Region
     */
    public Region deteriorate(final Random random, final double preservation) {
