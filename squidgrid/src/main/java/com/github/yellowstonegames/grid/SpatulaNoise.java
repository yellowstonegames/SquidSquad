...
public class SpatulaNoise implements INoise {
    ...
    public float getNoiseWithSeed(float x, float y, long seed) {
        ...
        float xc = xf * xf * (3f - 2f * xf);
        float yc = yf * yf * (3f - 2f * yf);
        ...
        final float xs = (1 - yc) * hobble(x0y0, x1y0, xf) + yc * hobble(x0y1, x1y1, xf);
        final float ys = (1 - xc) * hobble(x0y0, x0y1, yf) + xc * hobble(x1y0, x1y1, yf);
        return (xs + ys) * 0.5f;
    }
