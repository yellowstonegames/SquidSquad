...
import com.github.yellowstonegames.grid.DualMutantNoiseWrapper;
...
/**
 * Writes several world maps, including the data to load the WorldMapGenerators again, to the out/ folder.
 */
public class WorldLoopWriter extends ApplicationAdapter {
...
//    private static final int width = 300, height = 300;
    private static final int width = GLOBE_SPIN ? 256 : 800, height = GLOBE_SPIN ? 256 : 400;
...
//    private static final int SPACE = DescriptiveColorRgb.TRANSPARENT;
    ...
    public void create() {
        ...
        writer.setDitherAlgorithm(Dithered.DitherAlgorithm.BAYDIENT);
...
//        ((NoiseWrapper)iNoise.noise).setFractalSpiral(true);
        ...
        if(GLOBE_SPIN)
            ...
            world = new EllipticalWorldMap(seed, width, height, iNoise, 1f);
...
//        }
    ...
    public static void main(String[] arg) {
        ...
        config.setTitle("SquidSquad Demo: World Map Loop Writer");
        config.setWindowedMode(width * cellWidth, height * cellHeight);
        config.setResizable(false);
        config.useVsync(true);
        new Lwjgl3Application(new WorldLoopWriter(), config);
    }
}
