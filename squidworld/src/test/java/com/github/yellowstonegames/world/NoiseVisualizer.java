...
//        return Math.max(0f, n);
    ...
    public void create() {
        ...
        int[] gray244 = new int[244];
        for (int i = 0; i < 244; i++) {
            gray244[i] = (int)(255 * (i / 243f)) * 0x010101 << 8 | 0xFF;
        }

        gif = new AnimatedGif();
        gif.setDitherAlgorithm(Dithered.DitherAlgorithm.WREN);
        gif.setDitherStrength(0.2f);
        gif.palette = new PaletteReducer(gray244);
        System.arraycopy(palette, 0, gif.palette.paletteArray, 0, palette.length);
//                new int[]{
