 ...
 */
public class SimplexNoiseGV implements INoise {

    public long seed;
    public static final SimplexNoiseGV instance = new SimplexNoiseGV();

    public SimplexNoiseGV() {
        seed = 0x1337BEEF2A22L;
    }

    public SimplexNoiseGV(long seed)
    {
        this.seed = seed;
    }
...
//        return t / (0.5f + Math.abs(t));
    ...
    public SimplexNoiseGV deserializeFromString(String data) {
        seed = (DigitTools.longFromDec(data, 1, data.length() - 1));
        return this;
    }

    public static SimplexNoiseGV recreateFromString(String data) {
        return new SimplexNoiseGV(DigitTools.longFromDec(data, 1, data.length() - 1));
    }

    @Override
    public SimplexNoiseGV copy() {
        return new SimplexNoiseGV(seed);
    }
    ...
    public boolean equals(Object o) {
        ...
        SimplexNoiseGV that = (SimplexNoiseGV) o;
    ...
    /**
     * The lengths of gradient vectors from Noise:
     * <pre>
     * 2D gradient length: 1.0000000000
     * 3D gradient length: 1.2570662091
     * 4D gradient length: 2.5259992983
     * 5D gradient length: 2.0000000000
     * 6D gradient length: 1.0000000000
     * </pre>
     * The gradient vectors in GradientVectors are all unit-length.
     * @param args
     */
    public static void main(String[] args) {
        System.out.printf("2D gradient length: %.10f\n", Math.sqrt(
                Noise.GRADIENTS_2D[0] * Noise.GRADIENTS_2D[0] + Noise.GRADIENTS_2D[1] * Noise.GRADIENTS_2D[1]));
        System.out.printf("3D gradient length: %.10f\n", Math.sqrt(
                Noise.GRADIENTS_3D[0] * Noise.GRADIENTS_3D[0] + Noise.GRADIENTS_3D[1] * Noise.GRADIENTS_3D[1]
                        + Noise.GRADIENTS_3D[2] * Noise.GRADIENTS_3D[2]));
        System.out.printf("4D gradient length: %.10f\n", Math.sqrt(
                Noise.GRADIENTS_4D[0] * Noise.GRADIENTS_4D[0] + Noise.GRADIENTS_4D[1] * Noise.GRADIENTS_4D[1]
                        + Noise.GRADIENTS_4D[2] * Noise.GRADIENTS_4D[2] + Noise.GRADIENTS_4D[3] * Noise.GRADIENTS_4D[3]));
        System.out.printf("5D gradient length: %.10f\n", Math.sqrt(
                Noise.GRADIENTS_5D[0] * Noise.GRADIENTS_5D[0] + Noise.GRADIENTS_5D[1] * Noise.GRADIENTS_5D[1]
                        + Noise.GRADIENTS_5D[2] * Noise.GRADIENTS_5D[2] + Noise.GRADIENTS_5D[3] * Noise.GRADIENTS_5D[3]
                        + Noise.GRADIENTS_5D[4] * Noise.GRADIENTS_5D[4]));
        System.out.printf("6D gradient length: %.10f\n", Math.sqrt(
                Noise.GRADIENTS_6D[0] * Noise.GRADIENTS_6D[0] + Noise.GRADIENTS_6D[1] * Noise.GRADIENTS_6D[1]
                        + Noise.GRADIENTS_6D[2] * Noise.GRADIENTS_6D[2] + Noise.GRADIENTS_6D[3] * Noise.GRADIENTS_6D[3]
