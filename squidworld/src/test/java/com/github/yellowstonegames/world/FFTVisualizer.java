...
public class FFTVisualizer extends ApplicationAdapter {
    ...
    private final Interpolations.Interpolator watcher = new Interpolations.Interpolator("WATCHER", Interpolations.linearFunction){
        @Override
    ...
    private final float[] eq = new float[]{1f, PerlinNoise.calculateEqualizeAdjustment(1f)};
    ...
    private final CyclicNoise cyclic = new CyclicNoise(noise.getSeed(), 2);
    private final SorbetNoise sorbet = new SorbetNoise(noise.getSeed(), 2);
...
//        pm = new Pixmap(Gdx.files.local("out/blueNoise/BlueNoiseOmniTiling8x8.png"));
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                realKnown[x][y] = ((pm.getPixel(x & pm.getWidth() - 1, y & pm.getHeight() - 1) >> 24) + 0.5);
            }
        }
            ...
            public boolean keyUp(int keycode) {
                ...
                switch (keycode) {
                    ...
                    case W: // whirl, like a spiral
                        ...
                        break;
                    case M: // mash, affects equalizer
                        eq[0] *= (UIUtils.shift() ? 0.8f : 1.25f);
                        eq[1] = PerlinNoise.calculateEqualizeAdjustment(eq[0]);
                        break;
                    case P: // print
                        System.out.println("On mode: " + mode);
                        System.out.println("Dimensions: " + dim);
                        System.out.println("eq is " + eq[0] + ", " + eq[1]);
                        break;
                    case Q:
                    case ESCAPE: {
                        Gdx.app.exit();
...
//        threshold = (TimeUtils.millis() >>> 10 & 3) * 0x40p-8f + 0x20p-8f;
        ...
        float bright, nf = noise.getFrequency(), c = (paused ? startTime : TimeUtils.timeSinceMillis(startTime))
                * (noise.getNoiseType() >= Noise.MUTANT ? 0x3p-10f : 0x1p-10f) / nf, cc;
        double db;
        ...
        if(mode == 0) {
            ...
            switch (dim) {
                case 0:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = (float) (db = (1.0/255.0) * odd256(x, y, (int) noise.getSeed()));
                            real[x][y] = db;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
...
//                    for (int x = 0; x < width; x++) {
//                        for (int y = 0; y < height; y++) {
//                            bright = (float) (db = (1.0/255.0) * IntPointHash.hash256(x, y, (int)noise.getSeed()));
//                            real[x][y] = db;
//                            renderer.color(bright, bright, bright, 1f);
//                            renderer.vertex(x, y, 0);
//                        }
...
//                    break;
                case 1:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = (float) (db = (1.0/255.0) * LongPointHash.hash256(x, y, noise.getSeed()));
                            real[x][y] = db;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
...
//                    }
                ...
                default:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = (float) (db = (1.0/255.0) * castle256(x, y, noise.getSeed()));
                            real[x][y] = db;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
...
//                    for (int x = 0; x < width; x++) {
//                        for (int y = 0; y < height; y++) {
//                            bright = (float) (db = (1.0/255.0) * hash256(x, y, ctr, noise.getSeed()));
//                            real[x][y] = db;
//                            renderer.color(bright, bright, bright, 1f);
//                            renderer.vertex(x, y, 0);
//                        }
...
//                    for (int x = 0; x < width; x++) {
//                        for (int y = 0; y < height; y++) {
//                            bright = (float) (db = (1.0/255.0) * hash256(x, y, ctr, x + y - ctr, noise.getSeed()));
//                            real[x][y] = db;
//                            renderer.color(bright, bright, bright, 1f);
//                            renderer.vertex(x, y, 0);
//                        }
...
//                    for (int x = 0; x < width; x++) {
//                        for (int y = 0; y < height; y++) {
//                            bright = (float) (db = (1.0/255.0) * hash256(ctr + x, x - ctr, y - ctr,
...
//            }
            ...
            switch (dim){
                ...
                case 1:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
//                            bright = (float) (db = (1.0/255.0) * (BlueNoise.getSeeded(x, y, noise.getSeed()) + 128));
                            bright = (float) (db = (1.0/255.0) * (BlueNoise.getSeeded(x, y, (int)noise.getSeed()) + 128));
                            real[x][y] = db;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                case 2:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = (float) (db = (1.0/255.0) * (BlueNoise.getSeeded(x, y, (int)noise.getSeed()) & 255));
                            real[x][y] = db;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                default:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = (float) (db = (1.0/255.0) * (getSeededOmniTiling(x, y, (int)noise.getSeed()) + 128));
                            real[x][y] = db;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
            ...
            switch (dim){
                ...
                case 1:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = (float) (db = (1.0/255.0) * (BlueNoise.get(x, y, BlueNoise.TILE_TRI_NOISE[0]) + 128));
                            real[x][y] = db;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
...
//                    for (int x = 0; x < width; x++) {
//                        for (int y = 0; y < height; y++) {
//                            bright = (float) (db = (1.0/255.0) * (BlueNoise.getSeededTriangular(x, y, noise.getSeed()) + 128));
//                            real[x][y] = db;
//                            renderer.color(bright, bright, bright, 1f);
//                            renderer.vertex(x, y, 0);
//                        }
//                    }
                ...
                default:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = (float) (db = (1.0/255.0) * (BlueNoise.getSeededTriangular(x, y, (int)noise.getSeed()) + 128));
                            real[x][y] = db;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
...
//                        gif.write(Gdx.files.local("out/cube" + System.currentTimeMillis() + ".gif"), frames, 16);
            ...
            switch (dim) {
                case 0:
                    for (int x = 0; x < width; x++) {
                        ...
                        for (int y = 0; y < height; y++) {
                            points[dim][1] = (c + y) * fr;
                            bright = basicPrepare(perlin.getNoise(points[dim][0], points[dim][1]));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                case 1:
                    ...
                    for (int x = 0; x < width; x++) {
                        ...
                        for (int y = 0; y < height; y++) {
                            points[dim][1] = y * fr;
                            bright = basicPrepare(perlin.getNoise(points[dim][0], points[dim][1], points[dim][2]));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                case 2:
                    ...
                    for (int x = 0; x < width; x++) {
                        ...
                        for (int y = 0; y < height; y++) {
                            points[2][2] = TrigTools.cosTurns(y * iHeight) * 4 + cc;
                            points[2][3] = TrigTools.sinTurns(y * iHeight) * 4 + cc;
                            bright = basicPrepare(perlin.getNoise(points[dim][0], points[dim][1], points[dim][2], points[dim][3]));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                case 3:
                    ...
                    for (int x = 0; x < width; x++) {
                        ...
                        for (int y = 0; y < height; y++) {
                            points[3][2] = TrigTools.cosTurns(y * iHeight) * 4;
                            points[3][3] = TrigTools.sinTurns(y * iHeight) * 4;
                            bright = basicPrepare(perlin.getNoise(points[dim][0], points[dim][1], points[dim][2], points[dim][3], points[dim][4]));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
            ...
            switch (dim) {
                case 0:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = basicPrepare(adjustableNoise.getNoise(x + c, y + c));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                case 1:
                    for (int x = 0; x < width; x++) {
                        for (int y = 0; y < height; y++) {
                            bright = basicPrepare(adjustableNoise.getNoise(x, y, c));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                case 2:
                    for (int x = 0; x < width; x++) {
                        ...
                        for (int y = 0; y < height; y++) {
                            float yc = TrigTools.cosTurns(y * iHeight) * 64 + c, ys = TrigTools.sinTurns(y * iHeight) * 64 + c;
                            bright = basicPrepare(adjustableNoise.getNoise(xc, yc, xs, ys));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                case 3: {
                    for (int x = 0; x < width; x++) {
                        ...
                        for (int y = 0; y < height; y++) {
                            float yc = TrigTools.cosTurns(y * iHeight) * 64, ys = TrigTools.sinTurns(y * iHeight) * 64;
                            bright = basicPrepare(adjustableNoise.getNoise(xc, yc, xs, ys, c));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
                ...
                case 4: {
                    for (int x = 0; x < width; x++) {
                        ...
                        for (int y = 0; y < height; y++) {
                            float yc = TrigTools.cosTurns(y * iHeight) * 64 + c, ys = TrigTools.sinTurns(y * iHeight) * 64 + c,
                                    zc = TrigTools.cosTurns((x - y) * 0.5f * iWidth) * 64 - c, zs = TrigTools.sinTurns((x - y) * 0.5f * iWidth) * 64 - c;
                            bright = basicPrepare(adjustableNoise.getNoise(xc, yc, zc, xs, ys, zs));
                            real[x][y] = bright;
                            renderer.color(bright, bright, bright, 1f);
                            renderer.vertex(x, y, 0);
                        }
