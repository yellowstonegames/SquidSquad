...
```
api 'com.squidpony:squidgrid:4.0.10' // important code for anything that has a position here
api 'com.squidpony:squidplace:4.0.10' // allows generating dungeons, caves, wilderness areas as char arrays
api 'com.squidpony:squidstorepath:4.0.10' // adds a dependency for pathfinding and also allows saving related types 
api 'com.squidpony:squidstoretext:4.0.10' // adds a dependency for gibberish generation/translation; allows saving types
```

On all platforms but GWT, this will download everything SquidSquad needs, including `'com.squidpony:squidcore:4.0.10'`,
...
released version such as the current `4.0.10`, which can be obtained from the main source for dependencies on the
...
and look like `implementation 'com.squidpony:squidcore:4.0.10'`. Maven Central's search will still show (much) older
...
such release is `4.0.10`. You can always use a more recent build of SquidSquad, using JitPack to build a recent
