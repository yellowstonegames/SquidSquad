...
/**
 * An RNG that has a drastically longer period than the other generators in SquidLib
...
public final class LongPeriodRNG implements LegacyRandom {
    ...
    public String toString() {
        return "LongPeriodRNG with state hash 0x" + DigitTools.hexHash(state) + "L, choice 0x" + DigitTools.hex(choice);
    }
    ...
    public void setSelectedState(int selection, long value) {
        if(selection >= 0 && selection < 16)
            state[selection] = (value == 0L ? 1L : value);
        else
            choice = (int) (value & 15L);
    }
