 ...
 * <br>
 * You can use this like any other List, though it will contain ObTextEntry objects instead
 ...
 * you might expect from an ArrayList. Chiefly, this only considers the top-level Strings
...
//    protected int length = 0;
    ...
    private static void randomChars(long z, char[] mut, int offset)
    {
     ...
     * an {@link ObjectOrderedSet} of String. The returned set will retain the same order the Strings were entered in,
     * and you can use OrderedSet methods like {@link ObjectOrderedSet#getAt(int)} to look up keys by index. Changes to
     * the returned Set won't be reflected in this ObText.
     *
     * @return all unique top-level Strings (without associated values) as an OrderedSet of String keys
     */
    public ObjectOrderedSet<String> keyOrderedSet()
    {
        final int sz = size();
        ObjectOrderedSet<String> keys = new ObjectOrderedSet<>(sz);
        for (int i = 0; i < sz; i++) {
            keys.add(get(i).primary);
        }
        return keys;
    ...
    /**
     * Gets all unique Strings from the top level of this ObText as keys in an {@link ObjectObjectOrderedMap}, with the
     * first String associated with each key as its value (or null if nothing is associated with a key String). The
     * returned map will retain the same order the keys were entered in, and you can use ObjectObjectOrderedMap methods
     * like {@link ObjectObjectOrderedMap#keyAt(int)} to look up keys by index or
     * {@link ObjectObjectOrderedMap#getAt(int)} to look up value String by index.
     * Changes to the returned Map won't be reflected in this ObText.
     *
     * @return an OrderedMap of unique String keys associated with the first associated String for each key (or null)
     */
    public ObjectObjectOrderedMap<String, String> basicOrderedMap()
    {
        final int sz = size();
        ObjectObjectOrderedMap<String, String> keys = new ObjectObjectOrderedMap<>(sz);
        ObTextEntry got;
        for (int i = 0; i < sz; i++) {
            got = get(i);
    ...
    /**
     * Gets all unique Strings from the top level of this ObText as keys in an {@link ObjectObjectOrderedMap}, with any
     * Strings associated with those keys as their values (in a possibly-empty ArrayList of String for each value). The
     * returned map will retain the same order the keys were entered in, and you can use OrderedMap methods like
     * {@link ObjectObjectOrderedMap#keyAt(int)} to look up keys by index or {@link ObjectObjectOrderedMap#getAt(int)}
     * to look up the ArrayList of value Strings by index.
     * Changes to the returned Map won't be reflected in this ObText.
     *
     * @return an OrderedMap of unique String keys associated with ArrayList values containing associated Strings
     */
    public ObjectObjectOrderedMap<String, ArrayList<String>> shallowOrderedMap()
    {
        final int sz = size();
        ObjectObjectOrderedMap<String, ArrayList<String>> keys = new ObjectObjectOrderedMap<>(sz);
        ObTextEntry got;
        for (int i = 0; i < sz; i++) {
            got = get(i);
    ...
    public static class ContentMatcher extends Matcher {
         ...
         * You should typically call {@link #hasMatch()} even if {@code find()} returned true, to ensure there is a
         * valid String that can be acquired. This will return an empty String if hasMatch() returns false, but an empty
