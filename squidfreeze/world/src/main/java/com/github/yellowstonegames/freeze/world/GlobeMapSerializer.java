...
public class GlobeMapSerializer extends Serializer<GlobeMap> {
    public GlobeMapSerializer() {
        setImmutable(false);
        setAcceptsNull(false);
    }

    @Override
    public void write(final Kryo kryo, final Output output, final GlobeMap data) {
        output.writeString(data.stringSerialize());
    }

    @Override
    public GlobeMap read(final Kryo kryo, final Input input, final Class<? extends GlobeMap> dataClass) {
        return GlobeMap.recreateFromString(input.readString());
    }
}
