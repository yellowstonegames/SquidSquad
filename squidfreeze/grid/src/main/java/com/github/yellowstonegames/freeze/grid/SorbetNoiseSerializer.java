...
/**
 * Serializer for {@link SorbetNoise}; doesn't need anything else registered.
 */
public class SorbetNoiseSerializer extends Serializer<SorbetNoise> {
    ...
    public SorbetNoise read(final Kryo kryo, final Input input, final Class<? extends SorbetNoise> dataClass) {
        return new SorbetNoise(input.readLong(), input.readInt(true), input.readFloat());
    }

    @Override
    public SorbetNoise copy(Kryo kryo, SorbetNoise original) {
        return new SorbetNoise(original.getSeed(), original.getOctaves(), original.getFrequency());
    }
}
